...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || !$email || !$password) {
        ...
        if ($clientCheck->fetch()) {
            $error = 'Dit e-mailadres is al gekoppeld aan een klantaccount. <a href="' . BASE_PATH . '/login.php">Inloggen?</a>';
        } else {
...
<head>
  ...
  <style>
    body { display: flex; align-items: center; justify-content: center; min-height: 100vh; padding: 24px 0; background: var(--bg-2); }
    ...
    <div class="register-card">
      ...
      <?php else: ?>
        ...
        <p style="text-align:center;font-size:0.9rem;color:var(--text-muted);">
          Al een account? <a href="<?= BASE_PATH ?>/login.php">Inloggen</a>
        </p>
