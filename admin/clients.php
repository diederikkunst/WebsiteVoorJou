...
$stmt = $db->prepare("SELECT c.*, u.email AS user_email, (SELECT COUNT(*) FROM projects p WHERE p.client_id = c.id) AS project_count FROM clients c LEFT JOIN users u ON u.id = c.user_id $whereSQL ORDER BY c.created_at DESC");
$stmt->execute($params);
